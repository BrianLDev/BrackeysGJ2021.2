...
namespace RayFire
{
    ...
    public class RayfireUnyielding : MonoBehaviour
    {
        ...
        {
            // Get uny list
            connectivity.unyList = connectivity.GetComponents<RayfireUnyielding>().ToList();

            // Iterate very unyielding component
            if (connectivity.unyList != null && connectivity.unyList.Count > 0)
            {
                for (int i = 0; i < connectivity.unyList.Count; i++)
                    if (connectivity.unyList[i].enabled == true)
                    {
                        connectivity.unyList[i].connHost = connectivity;
                        GetConnectivityUnyRigidList (connectivity.unyList[i]);
                    }
            }
            
            // Set rigid list uny and sim states 
            SetConnectivityUnyRigidList (connectivity);
        }
        
        // Set uny state for mesh root rigids. Used by Mesh Root. Can be used for cluster shards
        static void GetConnectivityUnyRigidList(RayfireUnyielding uny)
        {
            // Get target mask
        ...
        // Set rigid list uny and sim states 
        public static void SetConnectivityUnyRigidList(RayfireConnectivity conn)
        {
            // Has no uny components
            if (conn.unyList == null)
                return;

            // Has no uny components
            if (conn.unyList.Count == 0)
                return;

            // Iterate uny components list
            for (int c = 0; c < conn.unyList.Count; c++)
            {
                // Disabled
                if (conn.unyList[c].enabled == false)
                    continue;

                // No rigids
                if (conn.unyList[c].rigidList.Count == 0)
                    continue;

                // Iterate cached rigids
                for (int i = 0; i < conn.unyList[c].rigidList.Count; i++)
                {
                    // Set uny state
                    SetRigidUnyState (conn.unyList[c].rigidList[i], conn.unyList[c].id, conn.unyList[c].unyielding, conn.unyList[c].activatable);

                    // Set simulation type
                    if (conn.unyList[c].simulationType != UnySimType.Original)
                    {
                        conn.unyList[c].rigidList[i].simulationType = (SimType)conn.unyList[c].simulationType;
                        RFPhysic.SetSimulationType (conn.unyList[c].rigidList[i].physics.rigidBody, conn.unyList[c].rigidList[i].simulationType,
                            ObjectType.Mesh, conn.unyList[c].rigidList[i].physics.useGravity, conn.unyList[c].rigidList[i].physics.solverIterations);
                    }
                }
            }
        }
        ...
        public static void SetRigidUnyState (RayfireRigid rigid, int unyId, bool unyielding, bool activatable)
        {
            ...
            if (unyielding == true && activatable == false)
            {
