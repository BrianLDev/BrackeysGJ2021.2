...
namespace RayFire
{
    public class RFBackupCluster
    {
        ...
        static bool BackupState (RayfireConnectivity scr, bool warning = false)
        {
            ...
            if (scr.rigidHost == null && scr.rigidRootHost == null)
                return false;
            
            // Rigid not mesh root
            if (scr.rigidHost != null && scr.rigidHost.objectType != ObjectType.MeshRoot)
                return false;
                
            // Backup disabled
            if (scr.rigidHost != null && scr.rigidHost.reset.connectivity == false)
            {
                if (warning == true)
        ...
        public static void RestoreConnectivity (RayfireConnectivity scr)
        {
            ...
            // Restore shards parent, position and rotation TODO should be in mesh Root reset
            ...
            if (scr.rigidHost != null)
            {
                ...
                // Reset uny data
                RayfireUnyielding.SetConnectivityUnyRigidList (scr);
            ...
            if (scr.rigidRootHost != null)
            {
                ...
                // Shards were cached, reinit non serialized vars, clear list otherwise
                if (RayfireConnectivity.InitCachedShardsByRigidRoot (scr.rigidRootHost, scr.cluster) == true)
                    scr.cluster.shards.Clear();
            }
