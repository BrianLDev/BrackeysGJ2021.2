...
namespace RayFire
{
    ...
    public class RFFade
    {
        ...
        public static IEnumerator FadeOffsetCor (RayfireRigidRoot scr)
        {
            ...
            while (scr.offsetFadeShards.Count > 0)
            {
                for (int i = scr.offsetFadeShards.Count - 1; i >= 0; i--)
                {
                    ...
                    if (Vector3.Distance (scr.offsetFadeShards[i].tm.position, scr.offsetFadeShards[i].pos) > scr.fading.byOffset)
                    {
        ...
        static public void SetOffsetFadeList (RayfireRigidRoot root)
        {
            ...
            {
                if (root.cluster.shards[i].rigid == null)
                {
                    if (root.fading.byOffset > 0)
                        root.offsetFadeShards.Add (root.cluster.shards[i]);
                }
                else
                {
                    if (root.cluster.shards[i].rigid.fading.byOffset > 0)
                        root.offsetFadeShards.Add (root.cluster.shards[i]);
                }
            }
        }
